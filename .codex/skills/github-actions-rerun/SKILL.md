...
5. 対象run/job、rerun範囲、理由を明示確認します。
6. failed jobsのみなら `gh run rerun <run-id> --repo OWNER/REPO --failed` を実行します。
7. 全jobなら `gh run rerun <run-id> --repo OWNER/REPO` を実行します。
8. 特定jobなら `gh run rerun <run-id> --repo OWNER/REPO --job <job-id>` を実行します。
9. 実行後に `gh run view <run-id> --repo OWNER/REPO` または新しいrunの表示で状態を確認します。
